...
### modified inheritence pattern for mitochondrial DNA.
...
#setwd("C:/Users/Sophie/Desktop/HZAM")
...
K_half <- 4236  # carrying capacity of entire range (for each sex); overall K is twice this, with sexes combined
max_generations <- 18;  # the time at which the simulation will stop 
#meandispersal <- 0.0216625  


# To get a sample of random data that follows a log-normal distribution and has arithmetic mean of X and a standard deviation of Y, you need to reparameterize things.  
# Roughly, you need to figure out what parameters should go into to the normal distribution such that when you exponentiate the draws, you end up with a mean of X and a standard deviation of Y.  
# Fortunately, that algebra has already been done and you can find the result on Wikipedia.  Here’s how you do it:

## Male
m <- 0.0711 #equivalent to 44.9km/631.78km  
s <- 0.1262 #equivalent to 79.75km/631.78km
### What happens if we cut dispersal by 25%?
#m <- 0.0533 #equivalent to 33.675km/631.78km  
#s <- 0.0947 #equivalent to 59.8125km/631.78km
### What happens if we cut dispersal in half?
#m <- 0.0355 # equivalent to 22.45/631.78
#s <- 0.0631 #equivalent to 39.875km/631.78km  
### What happens if we cut dispersal by 75%?
#m <- 0.0178 # equivalent to 11.225/631.78
#s <- 0.0316 #equivalent to 19.938km/631.78km 
### What happens if we cut dispersal by 90%?
#m <- 0.00711 # equivalent to 4.49/631.78
#s <- 0.01262 #equivalent to 7.975km/631.78km  
...
### What happens if we cut dispersal by 25%?
#m <- 0.0093 #equivalent to 5.88km/631.78km  
#s <- 0.0107 #equivalent to 6.78km/631.78km
### What happens if we cut dispersal in half?
#m <- 0.0062 #equivalent to 3.92km/631.78km
#s <- 0.0072 #equivalent to 4.52km/631.78km
### What happens if we cut dispersal by 75%?
#m <- 0.0031 #equivalent to 1.96km/631.78km
#s <- 0.0036 #equivalent to 2.26km/631.78km
### What happens if we cut dispersal by 90%?
#m <- 0.00124 #equivalent to 0.784km/631.78km
#s <- 0.00143 #equivalent to 0.904km/631.78km
location <- log(m^2 /sqrt(s^2 + m^2))
meanlog_F <- location
sdlog_F <- shape
shape <- sqrt(log(1 + (s^2 / m^2)))
...
hist(rlnorm(1000, meanlog_F, sdlog_F), breaks=60, col="red", add=TRUE)
...
movie_gen <- seq(1,20, by=1)  # applies only if making a movie
...
neutral_loci <- 1  # number of neutral loci (used for neutral measure of hybrid index)
          ...
          if (offspring >= 1)  { # if offspring, generate their location and genotypes
            for (kid in 1:offspring) {
              ...
              for (locus in 1:total_loci) {   
                ...
                kid_info[3+2*(locus-1)] <- pop_matrix_F[mother,1+2*(locus-1)+sample(2, 1)]
              }
...
#Merge Males and Female Matrices ONLY for the MtDNA and Nuclear DNA
Big_matrix_both <- rbind(Big_matrix_F, Big_matrix_M)

#Then convert it to a dataframe
Big_DF_both <- as.data.frame(Big_matrix_both)
colnames(Big_DF_both) <- c("Location", "Allele1", "Allele2", "RunID")
...
##quick plot
#plot(Big_DF_both$LocSummary, Big_DF_both$Allele1)
